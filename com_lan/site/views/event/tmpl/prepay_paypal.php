<?php defined( '_JEXEC' ) or die( 'Restricted access' );
	...
	$paypalEmail		= $params->paypal_email;// Event Setting
	$paypalAmount	 	= $params->cost_prepay;// Event Setting
	$paypalCurrency 	= $params->paypal_currency; // Event Setting
	//$paypalCancel		= JRoute::_('index.php?option=com_lan&view=event&id=' . $this->item->id); // Send to event page
	//$paypalReturn		= JRoute::_('index.php?option=com_lan&view=event&id=' . $this->item->id); // Send to event page
	$paypalItem			= 'Respawn LAN event ticket'; // Website Setting or Event Setting?
	$paypalItemNumber	= (int) $this->currentPlayer->id;
	$paypalNotifyURL	= 'http://beta.shadowreaper.net/respawn/components/com_lan/ipn/paypalIPN.php'; // COM_LAN or event setting
...
<tr class="row<?php echo $p % 2; ?>">
	<td class="center">	
		<form action="https://<?php echo $paypalSite; ?>/cgi-bin/webscr" method="post" target="_top">
			...
			<input type="image" src="https://<?php echo $paypalSite; ?>/en_AU/i/btn/btn_paynow_LG.gif" border="0" name="submit" alt="<?php echo JText::_("COM_LAN_EVENT_REGISTRATION_PREPAY_PAYPAL_DESC"); ?>">
			<img alt="" border="0" src="https://<?php echo $paypalSite; ?>/en_AU/i/scr/pixel.gif" width="1" height="1">
		</form>
	</td>
	<td class="left">	
		<?php echo JText::_('COM_LAN_EVENT_REGISTRATION_PREPAY_PAYPAL_DESC'); ?>
	</td>
</tr>