<div id="buttons" >
	<?php if(JFactory::getUser()->guest) { 
		...
		{
			if(isset($this->currentTeams->id))
			{
				echo $this->currentTeams->registered;
				if($this->currentTeams->status >= 0)
					if(strtotime($this->competition->competition_start) > time())
					{
						echo '<p><a class="btn btn-primary" onclick="unregisterCompetitionTeam()" href="javascript:void(0)" >' . JText::_('COM_EVENTS_COMPETITION_SUMMARY_TEAM_UNREGISTER_LABEL', true) . '</a> ';
					}
					else 
					{
						echo '<p><a class="btn" onclick="unregisterCompetitionTeam()" href="javascript:void(0)" >' . JText::_('COM_EVENTS_COMPETITION_SUMMARY_TEAM_FORFEIT_LABEL', true) . '</a> ';
					}
			}
			else
			{ 
				if(strtotime($this->competition->competition_start) > time())
				{
					echo '<p><a class="btn btn-primary" onclick="registerCompetitionTeam()" href="javascript:void(0)">' . JText::_('COM_EVENTS_COMPETITION_SUMMARY_TEAM_REGISTER_LABEL', true) . '</a> ';
				}
			}
		}
	}
