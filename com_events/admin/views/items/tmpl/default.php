...
<form action="<?php echo JRoute::_('index.php?option=com_events&view=items');?>" method="post" name="adminForm" id="adminForm">
	...
	<table class="adminlist table table-striped"">
		...
		<tbody>
			<?php foreach ($this->items as $i => $item) :
				...
				$canCreate	= $user->authorise('core.create',		'com_events.item.' . $item->id);
				$canEdit	= $user->authorise('core.edit',			'com_events.item.' . $item->id);
				$canCheckin = $user->authorise('core.manage',		'com_checkin') || $item->checked_out == $user->get('id') || $item-> checked_out == 0;
				$canChange	= $user->authorise('core.edit.state',	'com_events.item.' . $item->id) && $canCheckin;
			?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<?php if($item->checked_out) : ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'teams.', $canEdit); ?>
					<?php endif; ?>
					<?php if ($canCreate || $canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_events&task=item.edit&id=' . $item->id); ?>">
