	...
	<table class="adminlist table table-striped"">
		<thead>
			<tr>
				...
				<th width="10%" >
					...
					<?php echo JHTML::_('grid.order', $this->items, 'filesave.png', 'competitions.saveorder'); ?>
				</th>
				<th width="10%">
		...
		<tbody>
			<?php foreach ($this->items as $i => $item) :
				...
				$canEdit	= $user->authorise('core.edit',			'com_events.competition.' . $item->id);
				$canCheckin = $user->authorise('core.manage',		'com_checkin') || $item->checked_out == $user->get('id') || $item-> checked_out == 0;
				$canChange	= $user->authorise('core.edit.state',	'com_events.competition.' . $item->id) && $canCheckin;
			?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					<?php if($item->checked_out) : ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'competitions.', $canEdit); ?>
					<?php endif; ?>
					<?php if ($canCreate || $canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_events&task=competition.edit&id=' . $item->id); ?>">
				...
				<td class="center">
					<?php echo JHtml::_('jgrid.published', $item->published, $i, 'competitions.', $canChange); ?>
				</td>
				<td class="center">
				...
				<td class="center order"><!--"order"-->
					<?php if ($canChange) : ?>
						<!--<span><?php echo $this->pagination->orderUpIcon($i,
							...
							'competitions.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
						<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,
							($item->category_id == @$this->items[$i+1]->category_id),
							'competitions.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
