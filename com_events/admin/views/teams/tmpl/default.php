	...
	<table class="adminlist table table-striped"">
		<thead>
			<tr>
				...
				<th width="10%" >
					...
					<?php echo JHTML::_('grid.order', $this->items, 'filesave.png', 'teams.saveorder'); ?>
				</th>
				<th width="10%">
		...
		<tbody>
			<?php foreach ($this->items as $i => $item) :
				...
				$canEdit	= $user->authorise('core.edit',			'com_events.team.' . $item->id);
				$canCheckin = $user->authorise('core.manage',		'com_checkin') || $item->checked_out == $user->get('id') || $item-> checked_out == 0;
				$canChange	= $user->authorise('core.edit.state',	'com_events.team.' . $item->id) && $canCheckin;
			?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					...
					<a href="<?php echo JRoute::_('index.php?option=com_events&task=team.edit&id=' . $item->id); ?>">
				...
				<td class="center order"><!--"order"-->
					<?php if ($canChange) : ?>
						<!--<span><?php echo $this->pagination->orderUpIcon($i,
							...
							'teams.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
						<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total,
							($item->category_id == @$this->items[$i+1]->category_id),
							'teams.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
